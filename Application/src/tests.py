...
class UneClasseDeTest(unittest.TestCase):
    ...
    def main_test_diffusion(self):
        model = Model(delta=1, longueur=3, nb_cellules_large=5, rayon_ini=1)

        model.creer_concentrations(model.d_tore["nb_cellules_large"], model.d_cellulose["rayon_ini"])
    ...
    def test_set_concentration_by_coord(self):
        model = Model(delta=1, longueur=10, nb_cellules_large=10, c_min=110)
    ...
    def test_manger_bacterie(self):
        model = Model(delta=1, longueur=10, nb_cellules_large=10, c_min=0, c_ini=2)
